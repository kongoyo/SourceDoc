             ...
             CHGVAR     VAR(%BIN(&RCVVARLEN)) VALUE(140)
             CALLPRC    PRC('QtocRtvTCPA') PARM((&RCVVAR *BYREF +
                          *DFT) (&RCVVARLEN *BYREF *DFT) +
                          (&TCPAFORMAT *BYREF *DFT) (&APIERR *BYREF +
                          *DFT))

             CHGVAR     VAR(&TCPSTKSTS) VALUE(%SST(&RCVVAR 9 4))
             CHGVAR     VAR(&TCPSTKSTSN) VALUE(%BIN(&TCPSTKSTS))

             IF         COND(&TCPSTKSTSN *EQ 0) THEN(DO)
